...
package_target() {
  local TARGET="$1"
  local TARGET_DIR="$RELEASE_DIR/bandix-plus-$VERSION-$TARGET"
  local BINARY_PATH="target/$TARGET/release/bandix-plus"
  ...
  if [ -f "$BINARY_PATH" ]; then
    ...
    tar -czvf "$RELEASE_DIR/bandix-plus-$VERSION-$TARGET.tar.gz" -C "$RELEASE_DIR" "bandix-plus-$VERSION-$TARGET" > /dev/null
    rm -rf "$TARGET_DIR"
